...
func (u *User) AuthenticateWithPassword(password Password) error {
	...
	if !ok {
		return errors.Tracef(app.ErrBadRequest, "could not validate password")
	}
...
func (u *User) AuthenticateWithTOTP(totp TOTP) error {
	if !u.HasMFA() {
		return errors.Tracef(app.ErrBadRequest, "account does not have MFA")
	}

	switch status := u.AuthStatus(); status {
	case Unauthenticated:
		return errors.Tracef(app.ErrBadRequest, "TOTP cannot be used until authenticated with password")
	...
	default:
		if status != AwaitingMFA {
			return errors.Tracef(app.ErrBadRequest, "account is not awaiting MFA")
		}
	}
	...
	if !ok {
		return errors.Tracef(app.ErrBadRequest, "could not validate TOTP")
	}
