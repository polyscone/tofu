...
import (
	...
	"github.com/polyscone/tofu/internal/web/internal/token"
)
...
func RunTokenTests(t *testing.T, tokens token.Repo) {
	...
	_, err = tokens.Consume(ctx, tok1Old)
	...
	emailCmp, err := tokens.Consume(ctx, tok1)
	if err != nil {
		t.Fatal(err)
	}
